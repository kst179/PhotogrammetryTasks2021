                ...
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
...
#if SUBPIXEL_FITTING_ENABLE // такие тумблеры включающие/выключающие очередное улучшение алгоритма позволяют оценить какой вклад эта фича вносит в качество результата если в рамках уже готового алгоритма попробовать ее включить/выключить
                        {
                            ...
                            for (int k = 0; (k < SUBPIXEL_FITTING_MAX_ITER) && !found_good_point; ++k) {
                                prev = DoGPyramid[octave * OCTAVE_DOG_IMAGES + z - 1];
                                cur  = DoGPyramid[octave * OCTAVE_DOG_IMAGES + z];
                                next = DoGPyramid[octave * OCTAVE_DOG_IMAGES + z + 1];

                                center = cur.at<float>(y, x);

                                float dx = 0.5 * (cur.at<float>(y, x + 1) - cur.at<float>(y, x - 1));
                                float dy = 0.5 * (cur.at<float>(y + 1, x) - cur.at<float>(y - 1, x));
                                float dz = 0.5 * (next.at<float>(y, x) - prev.at<float>(y, x));
                                ...
                                float dzz = next.at<float>(y, x) + prev.at<float>(y, x) - 2 * cur.at<float>(y, x);

                                float dxy = 0.25 * (cur.at<float>(y + 1, x + 1) + cur.at<float>(y - 1, x - 1) -
                                        cur.at<float>(y + 1, x - 1) - cur.at<float>(y - 1, x + 1));
                                float dxz = 0.25 * (next.at<float>(y, x + 1) + prev.at<float>(y, x - 1) -
                                        next.at<float>(y, x - 1) - prev.at<float>(y, x + 1));
                                float dyz = 0.25 * (next.at<float>(y + 1, x) + prev.at<float>(y - 1, x) -
                                        prev.at<float>(y + 1, x) - next.at<float>(y - 1, x));

                                cv::Matx31f gradient(dx, dy, dz);
                                cv::Matx33f hessian(dxx, dxy, dxz,
                                                    dxy, dyy, dyz,
                                                    dxy, dyz, dzz);

                                cv::Matx31f shift;
                                cv::solve(hessian, -gradient, shift, cv::DECOMP_LU);

                                shift_x = shift(0, 0);
                                shift_y = shift(1, 0);
                                shift_z = shift(2, 0);

                                if (std::abs(shift_x) < 0.5 && std::abs(shift_y) < 0.5 && std::abs(shift_z) < 0.5) {
                                    ...
                                    if (0 > x - 1 || 0 > y - 1 || x + 1 >= cur.cols || y + 1 >= cur.rows || z < 1 || z > OCTAVE_NLAYERS - 1) {
                                        k = SUBPIXEL_FITTING_MAX_ITER;
                                    }
                                }
                            }
                            ...
                            float dx = cur.at<float>(y, x + 1) - cur.at<float>(y, x - 1);
                            float dy = cur.at<float>(y + 1, x) - cur.at<float>(y - 1, x);

                            dvalue = dx * shift_x + dy * shift_y;
                        }
#endif
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            ...
            double magnitude = std::sqrt(dIdx * dIdx + dIdy * dIdy);

            // orientation == theta
            // atan( (L(x, y + 1) − L(x, y − 1)) / (L(x + 1, y) − L(x − 1, y)) )
            double orientation = std::atan2(dIdy, dIdx);
                ...
                for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N; ++smpi) { // перебираем столбик очередного замера для текущей гистограммы
                    for (int smpy = 0; smpy < smpW; ++smpy) { // перебираем ряд пикселей текущего замера
                        for (int smpx = 0; smpx < smpW; ++smpx) { // перебираем столбик пикселей текущего замера
                            cv::Point2f shift(((-DESCRIPTOR_SIZE / 2.0 + hsti) * DESCRIPTOR_SAMPLES_N + smpi) * smpW + smpx,
                                              ((-DESCRIPTOR_SIZE / 2.0 + hstj) * DESCRIPTOR_SAMPLES_N + smpj) * smpW + smpy);
                            ...
                            size_t bin = std::floor(orientation / 360 * DESCRIPTOR_NBINS);
